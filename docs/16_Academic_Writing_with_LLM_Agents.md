...
## Require Reproducible Figures and Analyses

Treat every analysis and every manuscript figure as a reproducible research
artifact. It must be generated by an executable, version-controlled script that
starts from recorded inputs and produces the reported output. Do not accept a
figure copied from an interactive session, a manually edited image, or an
analysis whose steps exist only in a notebook's saved outputs or a person's
memory. A notebook may support exploration, but the accepted result needs a
version-controlled script or documented command that reproduces it.

Store an execution record for each accepted analysis or figure as JSON or a
durable log. Keep it beside the generated artifact or in the project's
experiment-record location, and link to it from the figure source or manuscript
task record. The record should identify:

- the generator script and its Git revision;
- the exact command and arguments, including random seeds where applicable;
- input data, intermediate artifacts, and configuration files, using stable
  paths or identifiers and checksums when available;
- the Python environment or other software versions needed to run the script;
- the output files produced; and
- the date, execution status, and any known limitations or manual review.

For example, a small JSON record for a manuscript figure might be saved as
`manuscript/figures/figure-3.record.json`:

```json
{
  "script": "scripts/plot_model_comparison.py",
  "git_revision": "<commit SHA>",
  "command": "python scripts/plot_model_comparison.py --results <results-id> --output manuscript/figures/figure-3.pdf",
  "arguments": {
    "results": "<stable result artifact ID>",
    "output": "manuscript/figures/figure-3.pdf",
    "seed": 1234
  },
  "inputs": [
    {
      "path_or_id": "<stable result artifact ID>",
      "checksum": "<optional checksum>"
    }
  ],
  "environment": "<environment lockfile, image, or version record>",
  "outputs": ["manuscript/figures/figure-3.pdf"],
  "status": "completed"
}
```

Do not commit restricted or very large inputs merely to satisfy this record.
Instead, record their approved storage location, version or immutable
identifier, and the access conditions needed to rerun the work. Regenerate the
figure or rerun the analysis after changing its script, inputs, configuration,
or environment, then update its execution record. A caption can explain a
figure, but it cannot substitute for the code and recorded inputs that produced
it.
...
- Accept each analysis and figure only when its version-controlled generator and
  JSON or log execution record identify the inputs, arguments, environment, and
  output files needed to reproduce it.
